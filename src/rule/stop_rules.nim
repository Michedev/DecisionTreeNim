...
proc min_samples_split_rule*(min_samples_split: int): Rule =
    proc min_samples_split_f(n: INode, X: MatrixView[float], y: VectorView[float]): bool {.gcsafe.} = 
        X.M <= min_samples_split
    return min_samples_split_f

proc max_depth_rule*(max_depth: int): Rule =
    proc max_depth_f(n: INode, X: MatrixView[float], y: VectorView[float]): bool {.gcsafe.} =
        return n.level > max_depth
    max_depth_f

proc unique_class_rule*(): Rule =
    proc unique_class_f(n: INode, X: MatrixView[float], y: VectorView[float]): bool {.gcsafe.} =
        let first_el = y[0]
        for el in y:
            if el != first_el:
                return false
...
#     proc min_impurity_decrease_f(n: INode, X: MatrixView[float], y: VectorView[float], X1: MatrixView[float], y1: VectorView[float], X2: MatrixView[float], y2: VectorView[float]): bool {.gcsafe.} =
